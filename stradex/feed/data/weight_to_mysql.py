...
def stradex_import_weight(connect, src_dir, market):
    ...
    for wgt_file in wgt_file_list:
        code = wgt_file.stem
        cur.execute("select stockid from `stradex_base`.`stock` where marketid=%s and code='%s'" % (marketid, code))
        ...
        with wgt_file.open('rb') as sourcefile:
            stockid = stockid[0]
            cur.execute("select date from `stradex_base`.`stkweight` where stockid=%s" % stockid)
    ...
    if records:
        cur.executemany("INSERT INTO `stradex_base`.`stkweight` (stockid, date, countAsGift, \
