...
TimeDelta::TimeDelta(int64_t days, int64_t hours, int64_t minutes, int64_t seconds,
    ...
    // 各参数添加限制，防止出现总和溢出的情况
    stradex_CHECK(days <= 99999999 && days >= -99999999, "Out of range! Input days: {}", days);
    stradex_CHECK(hours >= -100000 && hours <= 100000, "Out of range! Input hours: {}", hours);
    stradex_CHECK(minutes >= -100000 && minutes <= 100000, "Out of range! Input minutes: {}", minutes);
    stradex_CHECK(seconds >= -8639900 && seconds <= 8639900, "Out of range! Input seconds: {}",
              seconds);
    stradex_CHECK(milliseconds >= -86399000000 && milliseconds <= 86399000000,
              "Out of range! Input milliseconds: {}", milliseconds);
    stradex_CHECK(microseconds >= -86399000000 && microseconds <= 86399000000,
              "Out of range! Input microseconds: {}", microseconds);
    int64_t total =
      ((((days * 24 + hours) * 60 + minutes) * 60 + seconds) * 1000 + milliseconds) * 1000 +
      microseconds;
    stradex_CHECK(total >= m_min_micro_seconds && total <= m_max_micro_seconds, "Out of total range!");
...
TimeDelta TimeDelta::fromTicks(int64_t ticks) {
    stradex_CHECK(ticks >= m_min_micro_seconds && ticks <= m_max_micro_seconds, "Out of total range!");
...
TimeDelta stradex_API Hours(int64_t hours) {
    stradex_CHECK(hours >= TimeDelta::minTicks() / 3600000000LL &&
...
TimeDelta stradex_API Milliseconds(int64_t milliseconds) {
    stradex_CHECK(milliseconds >= TimeDelta::minTicks() / 1000LL &&
...
TimeDelta TimeDelta::operator/(double p) const {
    stradex_CHECK(p != 0, "Attempt to divide by 0!");
    return TimeDelta::fromTicks(static_cast<int64_t>(roundEx(double(ticks()) / p, 0)));
}
...
double TimeDelta::operator/(TimeDelta td) const {
    stradex_CHECK(td.ticks() != 0, "Attemp to divide by zero TimeDelta!");
    return double(ticks()) / double(td.ticks());
}

TimeDelta TimeDelta::operator%(TimeDelta td) const {
    stradex_CHECK(td.ticks() != 0, "Attemp to divide(mod) by zero TimeDelta!");
