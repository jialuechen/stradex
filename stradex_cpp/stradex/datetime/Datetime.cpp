...
stradex_API std::ostream& operator<<(std::ostream& out, const Datetime& d) {
    out << d.str();
    return out;
}
...
Datetime::Datetime(long year, long month, long day, long hh, long mm, long sec, long millisec,
                   long microsec) {
    stradex_CHECK(millisec >= 0 && millisec <= 999, "Out of range! millisec: {}", millisec);
    stradex_CHECK(microsec >= 0 && microsec <= 999, "Out of range! microsec: {}", microsec);
...
Datetime::Datetime(unsigned long long datetime) {
    ...
    if (datetime <= 99999999LL) {
        ...
        stradex_CHECK_THROW(hh < 24, std::out_of_range, "Hour value is out of rang 0..23");
        stradex_CHECK_THROW(mm < 60, std::out_of_range, "Minute value is out of range 0..59");
        bd::date d((unsigned short)year, (unsigned short)month, (unsigned short)day);
        m_data = bt::ptime(d, bt::time_duration((unsigned short)hh, (unsigned short)mm, 0));
    } else {
        stradex_THROW_EXCEPTION(std::out_of_range,
...
std::string Datetime::repr() const {
    stradex_IF_RETURN(isNull(), "Datetime()");
...
uint64_t Datetime::number() const noexcept {
    try {
        stradex_IF_RETURN(m_data.date() == bd::date(bd::pos_infin), Null<unsigned long long>());
...
long Datetime::year() const {
    stradex_CHECK_THROW(!isNull(), std::logic_error, "This is Null Datetime!");
...
Datetime Datetime::dateOfWeek(int day) const {
    stradex_IF_RETURN(*this == Null<Datetime>(), *this);
...
Datetime Datetime::startOfWeek() const {
    Datetime result;
    stradex_IF_RETURN(*this == Null<Datetime>(), result);
    int today = dayOfWeek();
    if (today == 0) {
...
Datetime Datetime::nextDay() const {
    stradex_IF_RETURN(*this == Null<Datetime>() || *this == Datetime::max(), *this);
...
Datetime Datetime::preDay() const {
    stradex_IF_RETURN(*this == Null<Datetime>() || *this == Datetime::min(), *this);
    return Datetime(date() - bd::date_duration(1));
}
