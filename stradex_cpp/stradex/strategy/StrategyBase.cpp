...
StrategyBase::StrategyBase(const string& name) {
    ...
    stradex_ERROR_IF(home == "", "Failed get user home path!");
#if stradex_OS_WINOWS
    m_config_file = format("{}\\{}", home, ".stradex\\stradex.ini");
#else
    m_config_file = format("{}/{}", home, ".stradex/stradex.ini");
#endif
    _initDefaultParam();
...
StrategyBase::~StrategyBase() {
    stradex_INFO("[Strategy {}] Quit Strategy!", m_name);
}
...
void StrategyBase::run() {
    ...
    stradex_INFO("[Strategy {}] strategy is running! You can press Ctrl-C to terminte ...", m_name);
    ...
    try {
        ...
        stradex_FATAL("[Strategy {}] Failed read configure file (\"{}\")! {}", m_name, m_config_file,
                  e.what());
        stradex_INFO("[Strategy {}] Exit Strategy", m_name);
        exit(1);
    } catch (...) {
        stradex_FATAL("[Strategy {}] Failed read configure file (\"{}\")! Unknow error!", m_name,
                  m_config_file);
        stradex_INFO("[Strategy {}] Exit Strategy", m_name);
        exit(1);
    }

    Parameter baseParam, blockParam, kdataParam, preloadParam, stradexParam;

    stradexParam.set<string>("tmpdir", config.get("stradex", "tmpdir", "."));
    stradexParam.set<string>("datadir", config.get("stradex", "datadir", "."));

    if (!config.hasSection("baseinfo")) {
        stradex_FATAL("Missing configure of baseinfo!");
        exit(1);
    }
    ...
    sm.init(baseParam, blockParam, kdataParam, preloadParam, stradexParam, m_context);
    ...
    for (const auto& code : stk_code_list) {
        ...
        if (!stk.isNull()) {
            ...
            stradex_WARN("[Strategy {}] Invalid code: {}, can't find the stock!", m_name, code);
        }
    }
    stradex_WARN_IF(m_stock_list.empty(), "[Strategy {}] stock list is empty!", m_name);
...
void StrategyBase::finishReceivedSpot(Datetime revTime) {
    stradex_IF_RETURN(m_stock_list.empty(), void());
...
void StrategyBase::_addTimer() {
    ...
    stradex_ERROR_IF_RETURN(openTime >= closeTime, void(), "Invalid market openTime: {}, closeTime: {}",
                        openTime, closeTime);
