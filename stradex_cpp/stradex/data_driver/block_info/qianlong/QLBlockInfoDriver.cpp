...
Block QLBlockInfoDriver ::getBlock(const string& category, const string& name) {
    ...
    stradex_ERROR_IF_RETURN(!haveParam("dir"), result, "Missing 'dir' param!");
    stradex_INFO_IF_RETURN(!haveParam(category), result, "No such category ({})!", category);

    string filename;
    try {
        filename = getParam<string>("dir") + "/" + getParam<string>(category);
    } catch (...) {
        stradex_ERROR("Maybe parameters errors!");
        return result;
    }

    std::ifstream inifile(filename.c_str(), std::ifstream::in);
    stradex_ERROR_IF_RETURN(!inifile, result, "Can't open file({})!", filename);
