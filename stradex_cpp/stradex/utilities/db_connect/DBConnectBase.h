...
#pragma once
#ifndef stradex_DB_CONNECT_DBCONNECTBASE_H
#define stradex_DB_CONNECT_DBCONNECTBASE_H
 ...
 * @ingroup DBConnect
 */
class stradex_API DBConnectBase {
    PARAMETER_SUPPORT
...
inline int DBConnectBase::queryInt(const string& query) {
    ...
    st->exec();
    stradex_CHECK((st->moveNext() && st->getNumColumns() == 1),
              "query doesn't result in exactly 1 element");
    int result = 0;
    st->getColumn(0, result);
    stradex_CHECK(!st->moveNext(), "query doesn't result in exactly 1 element");
    return result;
}
...
void DBConnectBase::save(T& item, bool autotrans) {
    ...
    try {
        ...
        stradex_THROW("failed save! sql: {}! {}", st->getSqlString(), e.what());

    } catch (...) {
        if (autotrans) {
            rollback();
        }
        stradex_THROW("failed save! sql: {}! Unknown error!", st->getSqlString());
    }
}
...
void DBConnectBase::batchSave(InputIterator first, InputIterator last, bool autotrans) {
    ...
    try {
        ...
        stradex_THROW("failed batch save! sql: {}! {}", st->getSqlString(), e.what());

    } catch (...) {
        if (autotrans) {
            rollback();
        }
        stradex_THROW("failed batch save! sql: {}! Unknown error!", st->getSqlString());
    }
}
...
void DBConnectBase::batchUpdate(InputIterator first, InputIterator last, bool autotrans) {
    ...
    try {
        ...
        stradex_THROW("failed batch update! sql: {}! {}", st->getSqlString(), e.what());
...
void DBConnectBase::remove(T& item, bool autotrans) {
    stradex_CHECK(item.id() != 0, "Invalid item, id is 0!");
    SQLStatementPtr st =
    ...
    try {
        ...
        stradex_THROW("failed delete! sql: {}! {}", st->getSqlString(), e.what());

    } catch (...) {
        if (autotrans) {
            rollback();
        }
        stradex_THROW("failed delete! sql: {}! Unknown error!", st->getSqlString());
    }
}
...
void DBConnectBase::batchRemove(InputIterator first, InputIterator last, bool autotrans) {
    ...
    try {
        ...
        stradex_THROW("failed batch delete! {}", e.what());

    } catch (...) {
        if (autotrans) {
            rollback();
        }
        stradex_THROW("failed batch delete! Unknown error!");
    }
}
...
inline void DBConnectBase::remove(const std::string& tablename, const std::string& where,
    ...
    try {
        ...
        stradex_THROW("Failed exec sql: {}! {}", sql, e.what());
    } catch (...) {
        if (autotrans) {
            rollback();
        }
        stradex_THROW(R"(Failed exec sql: {}! Unknown error!)", sql);
    }
}
