...
MySQLConnect::MySQLConnect(const Parameter& param) : DBConnectBase(param), m_mysql(nullptr) {
    try {
        m_mysql = new MYSQL;
        stradex_CHECK(mysql_init(m_mysql) != NULL, "Initial MySQL handle error!");
        ...
        stradex_TRACE("MYSQL host: {}", host);
        stradex_TRACE("MYSQL port: {}", port);
        stradex_TRACE("MYSQL database: {}", database);
        ...
        stradex_THROW("Failed allocate MySQLConnect! {}", e.what());

    } catch (stradex::exception&) {
        close();
        throw;

    } catch (std::exception& e) {
        close();
        stradex_THROW("Failed create MySQLConnent instance! {}", e.what());

    } catch (...) {
        close();
        stradex_THROW("Failed create MySQLConnect instance! Unknown error");
    }
}
...
bool MySQLConnect::ping() {
    stradex_IF_RETURN(!m_mysql, false);
    auto ret = mysql_ping(m_mysql);
    stradex_ERROR_IF_RETURN(ret, false, "mysql_ping error code: {}, msg: {}", ret,
                        mysql_error(m_mysql));
    return true;
}
...
void MySQLConnect::exec(const string& sql_string) {
    ...
    do {
        ...
        if (result) {
            ...
            // stradex_TRACE("num_fields: {}", num_fields);
            ...
            if (ret == 0) {
                ...
                stradex_TRACE("num_rows: {}", num_rows);
#endif
            } else {
