...
void stradex_API DBUpgrade(const DBConnectPtr &driver, const char *module_name,
    ...
    stradex_TRACE("check {} database version ...", module_name);
    ...
    if (version < start_version - 1) {
        stradex_ERROR("THe {} database is too old, can't upgrade!", module_name);
        return;
    }
    ...
    stradex_TRACE("current {} database version: {}", module_name, version);
    ...
    stradex_TRACE("update {} database ...", module_name);
