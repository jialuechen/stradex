...
public:
    ...
    int addFunc(Datetime start_date, Datetime end_date, TimeDelta start_time, TimeDelta end_time,
        ...
        stradex_CHECK(!start_date.isNull(), "Invalid start_date!");
        stradex_CHECK(!end_date.isNull(), "Invalid end_date!");
        Datetime start = start_date.startOfDay();
        Datetime end = end_date.startOfDay();
        stradex_CHECK(end >= start, "end_date({}) need > start_date({})!", end, start);
        stradex_CHECK(start_time >= TimeDelta(0) && start_time <= TimeDelta(0, 23, 59, 59, 999, 999),
                  "Invalid start_time: {}", start_time.repr());
        stradex_CHECK(end_time >= TimeDelta(0) && end_time <= TimeDelta(0, 23, 59, 59, 999, 999),
                  "Invalid end_time: {}", end_time.repr());
        stradex_CHECK(end_time >= start_time, "end_time({}) need >= start_time({})!", end_time,
                  start_time);
        stradex_CHECK(repeat_num > 0, "Invalid repeat_num: {}", repeat_num);
        stradex_CHECK(duration > TimeDelta(0), "Invalid duration: {}", duration.repr());
    ...
    int addDurationFunc(int repeat_num, TimeDelta duration, F&& f, Args&&... args) {
        stradex_CHECK(repeat_num > 0, "Invalid repeat_num: {}, must > 0", repeat_num);
        stradex_CHECK(duration > TimeDelta(), "Invalid duration: {}, must > TimeDelta(0)!",
                  duration.repr());
    ...
    int addDelayFunc(TimeDelta delay, F&& f, Args&&... args) {
        stradex_CHECK(delay > TimeDelta(), "Invalid delay: {}, must > TimeDelta(0)!", delay);
    ...
    int addFuncAtTime(Datetime time_point, F&& f, Args&&... args) {
        Datetime now = Datetime::now();
        stradex_CHECK(time_point > now, "You want run at {}, but now is {}", time_point, now);
    ...
    int addFuncAtTimeEveryDay(Datetime start_date, Datetime end_date, TimeDelta time, F&& f,
                              Args&&... args) {
        stradex_CHECK(!start_date.isNull() && !end_date.isNull(),
                  "Invalid start_date({}) or end_date({})!", start_date, end_date);
        stradex_CHECK(time >= TimeDelta() && time <= TimeDelta(0, 23, 59, 59, 999, 999),
                  "Invalid time {}", time.repr());
        Datetime start = start_date.startOfDay();
        Datetime end = end_date.startOfDay();
        stradex_CHECK(end >= start, "Invalid range of date! ({} - {})", start, end);
...
private:
    ...
    void detectThread() {
        while (!m_stop) {
            ...
            // stradex_TRACE("s.m_time_point: {}", s.m_time_point.repr());
            m_queue.push(s);
        }
    }
    ...
    int getNewTimerId() {
        ...
        stradex_WARN_IF_RETURN(m_timers.size() >= max_int, -1, "Timer queue is full!");
...
private:
    ...
    int _addFunc(Datetime start_date, Datetime end_date, TimeDelta start_time, TimeDelta end_time,
        ...
        Datetime now = Datetime::now();
        Datetime today = now.startOfDay();
        stradex_CHECK(end_date >= today, "Invalid end_date {}, because today is {}", end_date, today);
        if (end_date != Datetime::max()) {
            stradex_CHECK(end_date + end_time >= now,
        ...
        if (start_time < TimeDelta()) {
            ...
            if (first_start_time >= now) {
                ...
                stradex_CHECK(repeat_num > 1, "The time has expired! expect time {}, but now is {}",
        ...
        if (id < 0) {
            delete timer;
            lock.unlock();
            stradex_THROW("Failed to get new id, maybe too timers!");
        }
