...
double TradeManager::getHoldNumber(const Datetime& datetime, const Stock& stock) {
    // 日期小于账户建立日期，返回0
    stradex_IF_RETURN(datetime < m_init_datetime, 0.0);
...
price_t TradeManager::getDebtCash(const Datetime& datetime) {
    ...
    stradex_IF_RETURN(datetime >= lastDatetime(), m_borrow_cash);
...
TradeRecordList TradeManager::getTradeList(const Datetime& start_date,
    ...
    stradex_IF_RETURN(total == 0, result);
...
PositionRecord TradeManager::getPosition(const Datetime& datetime, const Stock& stock) {
    ...
    stradex_IF_RETURN(stock.isNull(), result);
    stradex_IF_RETURN(datetime < m_init_datetime, result);
    ...
    stradex_IF_RETURN(0.0 == number, result);
    ...
    stradex_WARN_IF(result.stock != stock, "Not found in the history positions, maybe exists error! {}",
                stock);
...
PositionRecord TradeManager::getShortPosition(const Stock& stock) const {
    stradex_IF_RETURN(stock.isNull(), PositionRecord());
...
bool TradeManager::checkin(const Datetime& datetime, price_t cash) {
    stradex_ERROR_IF_RETURN(cash <= 0.0, false, "{} cash({:<.3f}) must be > 0! ", datetime, cash);
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), false,
...
bool TradeManager::checkout(const Datetime& datetime, price_t cash) {
    stradex_ERROR_IF_RETURN(cash <= 0.0, false, "{} cash({:<.4f}) must be > 0! ", datetime, cash);
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), false,
                        "{} datetime must be >= lastDatetime({})!", datetime, lastDatetime());
    ...
    stradex_ERROR_IF_RETURN(out_cash > m_cash, false,
...
bool TradeManager::checkinStock(const Datetime& datetime, const Stock& stock, price_t price,
                                double number) {
    stradex_ERROR_IF_RETURN(stock.isNull(), false, "{} Try checkin Null stock!", datetime);
    stradex_ERROR_IF_RETURN(number == 0, false, "{} {} number is zero!", datetime, stock.market_code());
    stradex_ERROR_IF_RETURN(price <= 0, false, "{} {} price({:<.4f}) must be > 0!", datetime,
...
bool TradeManager::checkoutStock(const Datetime& datetime, const Stock& stock, price_t price,
                                 double number) {
    stradex_ERROR_IF_RETURN(stock.isNull(), false, "{} Try checkout Null stock!", datetime);
    stradex_ERROR_IF_RETURN(number == 0, false, "{} {} checkout number is zero!", datetime,
                        stock.market_code());
    stradex_ERROR_IF_RETURN(price <= 0.0, false, "{} {} checkout price({:<.4f}) must be > 0.0! ",
    ...
    // 当前是否有持仓
    position_map_type::iterator pos_iter = m_position.find(stock.id());
    stradex_ERROR_IF_RETURN(pos_iter == m_position.end(), false, "Try to checkout nonexistent stock!");

    PositionRecord& pos = pos_iter->second;
    // 取出数量超出了当前持仓数量
    stradex_ERROR_IF_RETURN(number > pos.number, false,
...
bool TradeManager::returnCash(const Datetime& datetime, price_t cash) {
    ...
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), false,
                        "{} datetime must be >= lastDatetime({})!", datetime, lastDatetime());
    stradex_ERROR_IF_RETURN(m_loan_list.empty(), false, "{} not borrow any cash!", datetime);
    stradex_ERROR_IF_RETURN(datetime < m_loan_list.back().datetime, false,
    ...
    // 欲归还的钱多余实际欠款
    stradex_ERROR_IF_RETURN(return_cash != 0.0, false, "{} return cash must <= borrowed cash!",
                        datetime);
...
bool TradeManager::borrowStock(const Datetime& datetime, const Stock& stock, price_t price,
    ...
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), false,
                        "{} {} datetime must be >= lastDatetime({})!", datetime,
                        stock.market_code(), lastDatetime());
    stradex_ERROR_IF_RETURN(number == 0, false, "{} {} Try to borrow number is zero!", datetime,
                        stock.market_code());
    stradex_ERROR_IF_RETURN(price <= 0.0, false, "{} {} price({:<.4f}) must be > 0!", datetime,
                        stock.market_code(), price);
...
bool TradeManager::returnStock(const Datetime& datetime, const Stock& stock, price_t price,
    ...
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), false,
                        "{} {} datetime must be >= lastDatetime({})!", datetime,
                        stock.market_code(), lastDatetime());
    stradex_ERROR_IF_RETURN(number == 0, false, "{} {} return stock number is zero!", datetime,
                        stock.market_code());
    stradex_ERROR_IF_RETURN(price <= 0.0, false, "{} {} price({:<.4f}) must be > 0!", datetime,
                        stock.market_code(), price);
    ...
    // 并未借入股票
    stradex_ERROR_IF_RETURN(bor_iter == m_borrow_stock.end(), false,
                        "{} {} Try to return nonborrowed stock! ", datetime, stock.market_code());

    BorrowRecord& bor = bor_iter->second;

    // 欲归还的数量大于借入的数量
    stradex_ERROR_IF_RETURN(number > bor.number, false,
...
TradeRecord TradeManager::buy(const Datetime& datetime, const Stock& stock, price_t realPrice,
    ...
    stradex_ERROR_IF_RETURN(stock.isNull(), result, "{} Stock is Null!", datetime);
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), result,
                        "{} {} datetime must be >= lastDatetime({})!", datetime,
                        stock.market_code(), lastDatetime());
    stradex_ERROR_IF_RETURN(number == 0.0, result, "{} {} numer is zero!", datetime,
                        stock.market_code());
    stradex_ERROR_IF_RETURN(number < stock.minTradeNumber(), result,
                        "{} {} Buy number({}) must be >= minTradeNumber({})!", datetime,
                        stock.market_code(), number, stock.minTradeNumber());
    stradex_ERROR_IF_RETURN(number > stock.maxTradeNumber(), result,
    ...
    if (krecord == Null<KRecord>()){
        stradex_ERROR(datetime << " " << stock.market_code()
    ...
    stradex_WARN_IF_RETURN(m_cash < roundEx(money + cost.total, precision), result,
...
TradeRecord TradeManager::sell(const Datetime& datetime, const Stock& stock, price_t realPrice,
    ...
    stradex_CHECK(!std::isnan(number), "sell number should be a valid double!");
    TradeRecord result;
    ...
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), result,
                        "{} {} datetime must be >= lastDatetime({})!", datetime,
                        stock.market_code(), lastDatetime());
    stradex_ERROR_IF_RETURN(number == 0.0, result, "{} {} number is zero!", datetime,
                        stock.market_code());
    ...
    stradex_ERROR_IF_RETURN(number < stock.minTradeNumber(), result,
                        ...
                        stock.market_code(), number, stock.minTradeNumber());
    stradex_ERROR_IF_RETURN(number != MAX_DOUBLE && number > stock.maxTradeNumber(), result,
                        "{} {} Sell number({}) must be <= maxTradeNumber({})!", datetime,
                        stock.market_code(), number, stock.maxTradeNumber());

    // 未持仓
    position_map_type::iterator pos_iter = m_position.find(stock.id());
    stradex_WARN_IF_RETURN(pos_iter == m_position.end(), result,
    ...
    // 欲卖出的数量大于当前持仓的数量
    stradex_ERROR_IF_RETURN(position.number < real_number, result,
...
TradeRecord TradeManager::sellShort(const Datetime& datetime, const Stock& stock, price_t realPrice,
    ...
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), result,
                        "{} {} datetime must be >= lastDatetime({})!", datetime,
                        stock.market_code(), lastDatetime());
    stradex_ERROR_IF_RETURN(number == 0, result, "{} {} numer is zero! ", datetime,
                        stock.market_code());
    stradex_ERROR_IF_RETURN(number < stock.minTradeNumber(), result,
                        "{} {} Buy number({}) must be >= minTradeNumber({})!", datetime,
                        stock.market_code(), number, stock.minTradeNumber());
    ...
    borrow_stock_map_type::const_iterator bor_iter;
    bor_iter = m_borrow_stock.find(stock.id());
    stradex_ERROR_IF_RETURN(bor_iter == m_borrow_stock.end(), result,
    ...
    if (pos_iter == m_short_position.end()) {
        ...
        // 借入的股票已经卖出过
        stradex_ERROR_IF_RETURN(pos_iter->second.number >= total_borrow_num, result,
...
TradeRecord TradeManager::buyShort(const Datetime& datetime, const Stock& stock, price_t realPrice,
    ...
    stradex_ERROR_IF_RETURN(datetime < lastDatetime(), result,
                        "{} {} datetime must be >= lastDatetime({})!", datetime,
                        stock.market_code(), lastDatetime());
    stradex_ERROR_IF_RETURN(number == 0, result, "{} {} number is zero!", datetime,
                        stock.market_code());
    stradex_ERROR_IF_RETURN(number < stock.minTradeNumber(), result,
    ...
    // 未持有空头仓位
    position_map_type::iterator pos_iter = m_short_position.find(stock.id());
    stradex_WARN_IF_RETURN(pos_iter == m_short_position.end(), result,
...
price_t TradeManager::cash(const Datetime& datetime, KQuery::KType ktype) {
    ...
    stradex_IF_RETURN(datetime == m_last_update_datetime, m_cash);
...
FundsRecord TradeManager::getFunds(const Datetime& indatetime, KQuery::KType ktype) {
    ...
    // stradex_IF_RETURN(indatetime == Null<Datetime>() || indatetime == lastDatetime(),
    // getFunds(ktype));
        ...
        switch (iter->business) {
            ...
            case BUSINESS_SELL:
                ...
                if (stock_iter != stock_map.end()) {
                    ...
                    stradex_WARN("{} {} Sell error in m_trade_list!", datetime,
            ...
            case BUSINESS_BUY_SHORT:
                ...
                if (short_stock_iter != short_stock_map.end()) {
                    ...
                    stradex_WARN("{} {} BuyShort Error in m_trade_list!", datetime,
                             iter->stock.market_code());
                }
                break;
            ...
            case BUSINESS_CHECKOUT_STOCK:
                ...
                if (stock_iter != stock_map.end()) {
                    ...
                    stradex_WARN("{} {} CheckoutStock Error in m_trade_list!", datetime,
                             iter->stock.market_code());
                }
            ...
            case BUSINESS_RETURN_STOCK:
                bor_stock_iter = bor_stock_map.find(iter->stock.id());
                if (bor_stock_iter == bor_stock_map.end()) {
                    stradex_WARN("{} {} Error return stock in m_trade_list!", iter->datetime,
                             iter->stock.market_code());
            ...
            default:
                stradex_WARN("{} {} Unknown business in m_trade_list!", datetime,
                         iter->stock.market_code());
                break;
        }
    }
...
void TradeManager::updateWithWeight(const Datetime& datetime) {
    stradex_IF_RETURN(datetime <= m_last_update_datetime, void());
...
void TradeManager::_saveAction(const TradeRecord& record) {
    stradex_IF_RETURN(getParam<bool>("save_action") == false, void());
...
void TradeManager::tocsv(const string& path) {
    ...
    stradex_ERROR_IF_RETURN(!file, void(), "Can't create file {}!", filename1);
    ...
    stradex_ERROR_IF_RETURN(!file, void(), "Can't create file {}!", filename2);
    ...
    stradex_ERROR_IF_RETURN(!file, void(), "Can't create file {}!", filename3);
    ...
    stradex_ERROR_IF_RETURN(!file, void(), "Can't create file {}!", filename4);
...
bool TradeManager::addTradeRecord(const TradeRecord& tr) {
    stradex_IF_RETURN(BUSINESS_INIT == tr.business, _add_init_tr(tr));
    stradex_ERROR_IF_RETURN(tr.datetime < lastDatetime(), false,
    ...
    switch (tr.business) {
        ...
        default:
            stradex_ERROR("tr.business is invalid({})!", int(tr.business));
            return false;
    }
...
bool TradeManager::_add_buy_tr(const TradeRecord& tr) {
    stradex_ERROR_IF_RETURN(tr.stock.isNull(), false, "tr.stock is null!");
    stradex_ERROR_IF_RETURN(tr.number == 0, false, "tr.number is zero!");
    stradex_ERROR_IF_RETURN(
    ...
    stradex_WARN_IF_RETURN(m_cash < roundEx(money + tr.cost.total, precision), false,
...
bool TradeManager::_add_sell_tr(const TradeRecord& tr) {
    ...
    // 未持仓
    position_map_type::iterator pos_iter = m_position.find(tr.stock.id());
    stradex_ERROR_IF_RETURN(pos_iter == m_position.end(), false, "No position!");

    PositionRecord& position = pos_iter->second;

    // 欲卖出的数量大于当前持仓的数量
    stradex_ERROR_IF_RETURN(position.number < tr.number, false, "Try sell number greater position!");
...
bool TradeManager::_add_checkin_tr(const TradeRecord& tr) {
    stradex_ERROR_IF_RETURN(tr.realPrice <= 0.0, false, "tr.realPrice <= 0.0!");
    int precision = getParam<int>("precision");
...
bool TradeManager::_add_checkout_tr(const TradeRecord& tr) {
    ...
    stradex_ERROR_IF_RETURN(out_cash > m_cash, false, "Checkout money > current cash!");
