 ...
 */
class stradex_API TradeManagerBase {
    PARAMETER_SUPPORT
...
public:
    ...
    virtual void _reset() {
        stradex_WARN("The subclass does not implement a reset method");
    }

    /** 执行 clone 操作 */
    shared_ptr<TradeManagerBase> clone() {
        shared_ptr<TradeManagerBase> p = _clone();
        stradex_CHECK(p, "Invalid ptr from _clone!");
        p->m_params = m_params;
        p->m_name = m_name;
    ...
    virtual void updateWithWeight(const Datetime& datetime) {
        stradex_WARN("The subclass does not implement a updateWithWeight method");
    }
    ...
    virtual double getMarginRate(const Datetime& datetime, const Stock& stock) {
        stradex_WARN("The subclass does not implement a getMarginRate method");
        return 0.0;
    }

    /** 初始资金 */
    virtual price_t initCash() const {
        stradex_WARN("The subclass does not implement this method");
        return 0.0;
    }
