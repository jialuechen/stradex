...
void save(Archive& ar, stradex::PriceList& values, unsigned int version) {
...
template <class Archive>
void load(Archive& ar, stradex::PriceList& values, unsigned int version) {
    size_t count = 0;
    unsigned int item_version = 0;
    ar& BOOST_SERIALIZATION_NVP(count);
    ar& BOOST_SERIALIZATION_NVP(item_version);
...
BOOST_SERIALIZATION_SPLIT_FREE(stradex::PriceList)
