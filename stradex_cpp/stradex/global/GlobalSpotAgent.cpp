...
void releaseGlobalSpotAgent() {
    if (g_spot_agent) {
        stradex_TRACE("relase spot agent");
        delete g_spot_agent;
...
static void updateStockDayData(const SpotRecord& spot) {
    ...
    stradex_IF_RETURN(stk.isNull(), void());
    stradex_IF_RETURN(!stk.isTransactionTime(spot.datetime), void());
...
static void updateStockDayUpData(const SpotRecord& spot, KQuery::KType ktype) {
    ...
    if (KQuery::WEEK == ktype) {
        ...
        stradex_THROW("Invalid ktype: {}", ktype);
    }
...
void stradex_API startSpotAgent(bool print) {
    auto& agent = *getGlobalSpotAgent();
    stradex_CHECK(!agent.isRunning(), "The agent is running, please stop first!");
