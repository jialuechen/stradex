...
stradex_API std::ostream& operator<<(std::ostream& os, const Portfolio& pf) {
    string strip(",\n");
    string space("  ");
    os << "Portfolio{\n"
...
stradex_API std::ostream& operator<<(std::ostream& os, const PortfolioPtr& pf) {
    if (pf) {
        os << *pf;
    } else {
        os << "Portfolio(NULL)";
...
bool Portfolio::_readyForRun() {
    if (!m_se) {
        stradex_WARN("m_se is null!");
        m_is_ready = false;
        return false;
    }

    if (!m_tm) {
        stradex_WARN("m_tm is null!");
        m_is_ready = false;
        return false;
    }

    if (!m_af) {
        stradex_WARN("m_am is null!");
        m_is_ready = false;
        return false;
    }

    // se算法和af算法不匹配时，给出告警
    stradex_WARN_IF(!m_se->isMatchAF(m_af), "The current SE and AF do not match!");
    ...
    for (size_t i = 0; i < total; i++) {
        ...
        stradex_CHECK(sys->readyForRun() && pro_sys->readyForRun(),
...
void Portfolio::_runMoment(const Datetime& date) {
    // 当前日期小于账户建立日期，直接忽略
    stradex_IF_RETURN(date < m_shadow_tm->initDatetime(), void());
...
void Portfolio::_runMomentOnClose(const Datetime& date) {
    ...
    if (getParam<bool>("trace") &&
        ...
        for (auto& sys : m_tmp_selected_list_on_open) {
            stradex_INFO("select on open: {}, cash: {}", sys->getTO().getStock(),
                     sys->getTM()->cash(date, m_query.kType()));
        }
        for (auto& sys : m_tmp_selected_list_on_close) {
            stradex_INFO("select on close: {}, cash: {}", sys->getTO().getStock(),
                     sys->getTM()->cash(date, m_query.kType()));
        }
    }
...
void Portfolio::run(const KQuery& query, bool force) {
    ...
    stradex_IF_RETURN(!m_need_calculate, void());
    stradex_ERROR_IF_RETURN(!_readyForRun(), void(),
