...
stradex_API std::ostream& operator<<(std::ostream& os, const AllocateFundsBase& af) {
    os << "AllocateFunds(" << af.name() << ", " << af.getParameter() << ")";
    return os;
}

stradex_API std::ostream& operator<<(std::ostream& os, const AFPtr& af) {
    if (af) {
        os << *af;
    } else {
...
void AllocateFundsBase::reset() {
    ...
    // 参数检查
    stradex_ERROR_IF(getParam<int>("max_sys_num") <= 0, R"(AF param["max_sys_num"]({}) need > 0!)",
    ...
    stradex_CHECK(getParam<double>("default_reserve_percent") >= 0.0,
...
AFPtr AllocateFundsBase::clone() {
    ...
    if (!p || p.get() == this) {
        stradex_WARN("Failed clone! Will use self-ptr!");
...
void AllocateFundsBase::setReservePercent(double percent) {
    stradex_CHECK_THROW(percent >= 0.0 && percent <= 1.0, std::out_of_range,
...
bool AllocateFundsBase::_returnAssets(const SYSPtr& sys, const Datetime& date) {
    ...
    stradex_IF_RETURN(pos == Null<size_t>(), false);
...
void AllocateFundsBase::_adjust_with_running(const Datetime& date, const SystemList& se_list,
    ...
    stradex_IF_RETURN(sw_list.size() == 0, void());
    ...
    // 计算需保留的资产
    stradex_ERROR_IF(m_reserve_percent < 0.0,
    ...
    stradex_CHECK(weight_unit > 0, "Invalid weight_unit! {}", weight_unit);
...
void AllocateFundsBase::_adjust_without_running(const Datetime& date, const SystemList& se_list,
                                                const std::list<SYSPtr>& running_list) {
    stradex_IF_RETURN(se_list.size() == 0, void());

    //如果运行中的系统数已大于等于允许的最大系统数，直接返回
    int max_num = getParam<int>("max_sys_num");
    stradex_IF_RETURN(running_list.size() >= max_num, void());
    ...
    stradex_IF_RETURN(m_shadow_tm->cash(date, m_query.kType()) <= reserve_funds, void());
