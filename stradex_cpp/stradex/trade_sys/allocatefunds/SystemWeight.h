 ...
 */
class stradex_API SystemWeight {
public:
    /** 默认构造函数，缺省权重为1.0 */
    ...
    SystemWeight(const SystemPtr& sys, double weight) : m_sys(sys), m_weight(1.0) {
        stradex_CHECK_THROW(weight >= 0.0 && weight <= 1.0, std::out_of_range,
...
stradex_API std::ostream& operator<<(std::ostream&, const SystemWeight&);
...
template <>
struct fmt::formatter<stradex::SystemWeight> : ostream_formatter {};
#endif
