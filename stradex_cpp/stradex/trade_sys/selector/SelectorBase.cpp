...
stradex_API std::ostream& operator<<(std::ostream& os, const SelectorBase& st) {
    os << "Selector(" << st.name() << ", " << st.getParameter() << ")";
    return os;
}

stradex_API std::ostream& operator<<(std::ostream& os, const SelectorPtr& st) {
    if (st) {
        os << *st;
    } else {
        os << "Selector(NULL)";
...
bool SelectorBase::addStock(const Stock& stock, const SystemPtr& protoSys) {
    stradex_ERROR_IF_RETURN(stock.isNull(), false, "Try add Null stock, will be discard!");
    stradex_ERROR_IF_RETURN(!protoSys, false, "Try add Null protoSys, will be discard!");
    stradex_ERROR_IF_RETURN(!protoSys->getMM(), false, "protoSys has not MoneyManager!");
    stradex_ERROR_IF_RETURN(!protoSys->getSG(), false, "protoSys has not Siganl!");
...
bool SelectorBase::addStockList(const StockList& stkList, const SystemPtr& protoSys) {
    ...
    for (; iter != stkList.end(); ++iter) {
        if (iter->isNull()) {
            stradex_WARN("Try add Null stock, will be discard!");
            continue;
        }
