...
class stradex_API Portfolio;

/**
 * 交易对象选择模块
 * @ingroup Selector
 */
class stradex_API SelectorBase : public enable_shared_from_this<SelectorBase> {
    PARAMETER_SUPPORT
...
private:
    friend class stradex_API Portfolio;
...
stradex_API std::ostream& operator<<(std::ostream&, const SelectorBase&);
stradex_API std::ostream& operator<<(std::ostream&, const SelectorPtr&);
...
template <>
struct fmt::formatter<stradex::SelectorBase> : ostream_formatter {};

template <>
struct fmt::formatter<stradex::SelectorPtr> : ostream_formatter {};
#endif
