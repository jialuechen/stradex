...
stradex_API std::ostream& operator<<(std::ostream& os, const System& sys) {
    string strip(",\n");
    string space("  ");
    os << "System{\n"
...
stradex_API std::ostream& operator<<(std::ostream& os, const SystemPtr& sys) {
    if (sys) {
        os << *sys;
    } else {
        os << "System(NULL)";
...
bool System::readyForRun() {
    stradex_ERROR_IF_RETURN(!m_tm, false, "Not setTradeManager!");
    stradex_ERROR_IF_RETURN(!m_mm, false, "Not setMoneyManager!");
    stradex_ERROR_IF_RETURN(!m_sg, false, "Not setSignal!");
...
void System::run(const KQuery& query, bool reset) {
    stradex_ERROR_IF_RETURN(m_stock.isNull(), void(), "m_stock is NULL!");

    // reset必须在readyForRun之前，否则m_pre_cn_valid、m_pre_ev_valid将会被赋为错误的初值
    if (reset)
        this->reset(true, true);

    stradex_IF_RETURN(!readyForRun(), void());

    // m_stock = stock; 在setTO里赋值
    KData kdata = m_stock.getKData(query);
    stradex_IF_RETURN(kdata.empty(), void());
...
void System::run(const KData& kdata, bool reset) {
    stradex_INFO_IF_RETURN(kdata.empty(), void(), "Input kdata is empty!");
...
TradeRecord System::runMoment(const Datetime& datetime) {
    ...
    stradex_IF_RETURN(pos == Null<size_t>(), TradeRecord());
...
TradeRecord System::sellForce(const KRecord& today, const KRecord& src_today, double num,
                              Part from) {
    stradex_ASSERT_M(from == PART_ALLOCATEFUNDS || from == PART_PORTFOLIO,
...
TradeRecord System::_processRequest(const KRecord& today, const KRecord& src_today) {
    stradex_IF_RETURN(m_buyRequest.valid, _buyDelay(today, src_today));
    stradex_IF_RETURN(m_sellRequest.valid, _sellDelay(today, src_today));
    stradex_IF_RETURN(m_sellShortRequest.valid, _sellShortDelay(today, src_today));
    stradex_IF_RETURN(m_buyShortRequest.valid, _buyShortDelay(today, src_today));
    return TradeRecord();
}

price_t System::_getStoplossPrice(const KRecord& today, const KRecord& src_today, price_t price) {
    stradex_IF_RETURN(!m_st, 0.0);
    stradex_IF_RETURN(today.highPrice == today.lowPrice, src_today.lowPrice);
