...
bool KDataTempCsvDriver::getIndexRangeByDate(const string& market, const string& code,
    ...
    STRADEX_IF_RETURN(start_date >= end_date, false);
...
KRecordList KDataTempCsvDriver::getKRecordList(const string& market, const string& code,
    ...
    if (query.queryType() == KQuery::INDEX) {
        ...
        STRADEX_INFO("Query by date are not supported!");
    }
    return result;
}
...
KRecordList KDataTempCsvDriver::_getKRecordListByIndex(const string& market, const string& code,
    ...
    if (kType == KQuery::DAY) {
        ...
        STRADEX_INFO("Only support DAY and MIN!");
        return result;
    }

    std::ifstream infile(filename.c_str());
    STRADEX_ERROR_IF_RETURN(!infile, result, "Can't open this file: {}", filename);
    string line;
    ...
    while (std::getline(infile, line)) {
        ...
        try {
            ...
            STRADEX_WARN("Invalid data in line {}! at trans {}", line_no, action);
        }
