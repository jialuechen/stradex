...
PriceList HistoryFinanceReader ::getHistoryFinanceInfo(Datetime date, const string& market,
    ...
    STRADEX_INFO_IF_RETURN(NULL == fp, result, "Can't found {}", filename);
    ...
    if (!fread(header_buf, 1, 20, fp)) {
        STRADEX_ERROR("read data failed! {}", filename);
        fclose(fp);
        return result;
    }
    ...
    for (int i = 0; i < max_count; i++) {
        if (!fread(stock_code, 1, 7, fp)) {
            STRADEX_ERROR("read stock_code failed! {}", filename);
            fclose(fp);
            return result;
        }

        if (!fread(&address, 4, 1, fp)) {
            STRADEX_ERROR("read stock_item address failed! {}", filename);
            fclose(fp);
            return result;
        }
    ...
    if (address != 0) {
        ...
        if (report_fields_count >= MAX_COL_NUM) {
            STRADEX_WARN("Over MAX_COL_NUM! {}", filename);
        ...
        if (!fread(result_buffer, 4, report_fields_count, fp)) {
            STRADEX_ERROR("read col data failed! {}", filename);
            fclose(fp);
            return result;
        }
        ...
        STRADEX_ERROR("Invalid address(0)! {}", filename);
    }
